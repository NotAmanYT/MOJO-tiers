async function loadTiers(mode) {
    ...
    order.forEach(rank => {
        if (data[rank] && data[rank].length > 0) {
            ...
            row.innerHTML = `<div class="rank-label">${rank}</div><div class="player-grid">${data[rank].map(p => `<div class="player-tag"><img src="https://minotar.net/helm/${p}/24.png"><span>${p}</span></div>`).join('')}</div>`;
